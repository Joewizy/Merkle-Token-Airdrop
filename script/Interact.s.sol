...
contract ClaimAirdrop is Script{
    error __ClaimAirdropScript__InvalidLengthSignature();

    address CLAIMING_ADDRESS = 0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266; // Default Anvil KEY
    ...
    function splitSignature(bytes memory sig) public returns(uint8 v, bytes32 r, bytes32 s){
        if(sig.length != 65){
            revert __ClaimAirdropScript__InvalidLengthSignature();
        }
        assembly {
            r:= mload(add(sig, 32))  // Load 32 bytes for r
            s:= mload(add(sig, 64))  // Load 32 bytes for s
            v:= byte(0, mload(add(sig, 96))) // Load the first byte for v 
        }
    }

    function run() external{
        address mostRecentlyDeployed = DevOpsTools.get_most_recent_deployment("Merkle Airdrop", block.chainid);
